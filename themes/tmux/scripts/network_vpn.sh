...
vpn_function() {
  ...
  Linux)
    ...
    verbose=$(get_tmux_option "@hanabi-network-vpn-verbose" false)
    ...
    if [[ $vpn =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]]; then
      if $verbose; then
        vpn_label=$(get_tmux_option "@hanabi-network-vpn-label" "󰌘 ")
        echo "$vpn_label$vpn"
      else
        vpn_label=$(get_tmux_option "@hanabi-network-vpn-label" "VPN-ON")
        echo "$vpn_label"
      fi
      ...
      if [[ -z $node ]] || [[ "$node"  == 'null' ]]; then
        ...
        if $verbose; then
          ...
          vpn_label=$(get_tmux_option "@hanabi-network-vpn-label" "Tailscale")
          echo "$vpn_label"
        fi
      fi
    else
  ...
  Darwin)
    ...
    if [ -z "$vpn" ]; then
      ...
      if $verbose; then
        ...
        vpn_label=$(get_tmux_option "@hanabi-network-vpn-label" "VPN")
        echo "$vpn_label"
      fi
    fi
    ;;
