...
main() {
  ...
  PLAY_ICON=$(get_tmux_option "@hanabi-mac-player-play-icon" "♪")
  PAUSE_ICON=$(get_tmux_option "@hanabi-mac-player-pause-icon" "❚❚ ")
  MAX_LENGTH=$(get_tmux_option "@hanabi-mac-player-length" 25)

  # Remote variables
  REMOTE_ACCESS=$(get_tmux_option "@hanabi-mac-player-remote" "false")
  REMOTE_APP=$(get_tmux_option "@hanabi-mac-player-app" "Spotify")
  ...
  if [[ "$REMOTE_ACCESS" == "true" ]]; then
    ...
    PLAY_PAUSE_BUTTON=$(get_tmux_option "@hanabi-mac-player-remote-play-pause" "P")
    BACK_BUTTON=$(get_tmux_option "@hanabi-mac-player-remote-back" "R")
    NEXT_BUTTON=$(get_tmux_option "@hanabi-mac-player-remote-next" "N")

    remoteControl "$PLAY_PAUSE_BUTTON" "$BACK_BUTTON" "$NEXT_BUTTON" "$REMOTE_APP"
  else
    # Clean up when remote is disabled
    tmux set -g @hanabi-mac-player-remote-play-pause ""
    tmux set -g @hanabi-mac-player-remote-back ""
    tmux set -g @hanabi-mac-player-remote-next ""
