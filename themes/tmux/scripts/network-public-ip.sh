...
DATAFILE="/tmp/.hanabi-tmux-public-ip-data"
LAST_EXEC_FILE="/tmp/.hanabi-tmux-public-ip-last-exec"
INTERVAL=1200
...
main() {
  ...
  if (((_now - _last) > INTERVAL)); then
    ...
    network_public_ip_label=$(get_tmux_option "@hanabi-network-public-ip-label" "IP")
