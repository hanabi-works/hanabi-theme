...
main() {
  ...
  MAX_LENGTH="$(get_tmux_option "@hanabi-spr-length" 25)"

  # Remote Control checker
  SPR_REMOTE_ACCESS="$(get_tmux_option "@hanabi-spr-remote" "false")"

  PLAY_ICON=$(get_tmux_option "@hanabi-spr-play-icon" "♪ ")
  PAUSE_ICON=$(get_tmux_option "@hanabi-spr-pause-icon" "❚❚ ")
  ...
  if [[ "$SPR_REMOTE_ACCESS" == "true" ]]; then
    PLAY_PAUSE_BUTTON=$(get_tmux_option "@hanabi-spr-remote-play-pause" "P")
    BACK_BUTTON=$(get_tmux_option "@hanabi-spr-remote-back" "R")
    NEXT_BUTTON=$(get_tmux_option "@hanabi-spr-remote-next" "N")
    sprRemoteControl "$PLAY_PAUSE_BUTTON" "$BACK_BUTTON" "$NEXT_BUTTON"
  else
    tmux set -g @hanabi-spr-remote-play-pause ""
    tmux set -g @hanabi-spr-remote-back ""
    tmux set -g @hanabi-spr-remote-next ""
