...
get_platform()
{
  case $(uname -s) in
    Linux)
      ...
      gpu_label=$(get_tmux_option "@hanabi-force-gpu" false)
...
{
  gpu=$(get_platform)
  gpu_vram_percent=$(get_tmux_option "@hanabi-gpu-vram-percent" false)
  if [[ "$gpu" == NVIDIA ]]; then
    ...
    exit 0
      ...
      used_accuracy=$(get_tmux_option "@hanabi-gpu-vram-used-accuracy" "d")
      total_accuracy=$(get_tmux_option "@hanabi-gpu-vram-total-accuracy" "d")
...
main()
{
  ...
  gpu_label=$(get_tmux_option "@hanabi-gpu-vram-label" "VRAM")
  gpu_usage=$(get_gpu)
