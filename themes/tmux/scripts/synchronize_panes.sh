...
main()
{
  ...
  # Use the @hanabi-synchronize-panes-refresh-rate plugin variable to override it.
  RATE_OVERRIDE=$(get_tmux_option "@hanabi-synchronize-panes-refresh-rate" "")
  if [[ -n "$RATE_OVERRIDE" ]]; then
    RATE="$RATE_OVERRIDE"
  fi

  synchronize_panes_auto_hide=$(get_tmux_option "@hanabi-synchronize-panes-auto-hide" "false")
